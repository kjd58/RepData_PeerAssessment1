...
The mean number of steps taken per day is `r mean_steps_per_day` and the median is `r median_steps_per_day`.
...
The maximum number of steps, averaged across all the days in the dataset, is `r m_steps` which occurrs in the interval `r m_interval`.
...
The number of missing step values is `r missing`.
...
The mean number of steps taken per day is `r full_mean_steps_per_day` and the median is `r full_median_steps_per_day`.
...
The differences between the means is `r mean_diff` and the differences betwen the two medians is `r median_diff`. As these values are zero and near zero the imputing of the missing values has have no effect on the observations.
